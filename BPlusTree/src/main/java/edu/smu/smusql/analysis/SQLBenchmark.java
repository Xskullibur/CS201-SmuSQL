...
public class SQLBenchmark {
    ...
    public static void main(String[] args) {
        ...
        // Add other engines with no configs
        // engineConfigs.put(EngineType.HASH_MAP, Collections.singletonList(new EngineConfig.Builder().build()));
        ...
        for (Map.Entry<EngineType, List<EngineConfig>> entry : engineConfigs.entrySet()) {
            ...
            for (EngineConfig config : configs) {
                ...
                String fileName = String.format("%s_%s_results.csv",
                    engineType.name().toLowerCase(),
                    configStr);

                System.out.printf("Starting benchmark for %s with config: %s%n",
                    engineType, configStr);

                SQLBenchmark benchmark = new SQLBenchmark(
                    engineType,
                    config,
                    seed,
                    fileName
    ...
    private static String getConfigString(EngineType engineType, EngineConfig config) {
        switch (engineType) {
            case BPLUS:
                return config.getParameter("useCache") != null &&
                    (boolean) config.getParameter("useCache") ? "withcache" : "nocache";
    ...
    public void runBenchmark(int numberOfQueries) {
        ...
        System.out.println("Starting benchmark with " + numberOfQueries +
            " queries on engine: " + engine.getClass().getSimpleName());
    ...
    private void prepopulateTables() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            engine.executeSQL(String.format(
                "INSERT INTO users VALUES (%d, '%s', %d, '%s')",
                id, name, age, city
            ));
        }
        ...
        for (int i = 0; i < 100; i++) {
            ...
            engine.executeSQL(String.format(
                "INSERT INTO products VALUES (%d, '%s', %.2f, '%s')",
    ...
    private void executeRangeSelect() {
        if (random.nextBoolean()) {
            ...
            engine.executeSQL(String.format(
                "SELECT * FROM users WHERE age >= %d AND age <= %d",
                minAge, maxAge
            ));
        } else {
            // Price range query
            ...
            engine.executeSQL(String.format(
                "SELECT * FROM products WHERE price >= %.2f AND price <= %.2f",
                minPrice, maxPrice
            ));
        }
    }
    ...
    private void executeRangeUpdate() {
        if (random.nextBoolean()) {
            ...
            engine.executeSQL(String.format(
                "UPDATE users SET age = %d WHERE age >= %d AND age <= %d",
    ...
    private void executeEqualsUpdate() {
        if (random.nextBoolean()) {
            ...
            int newAge = random.nextInt(60) + 20;
            engine.executeSQL(String.format(
                "UPDATE users SET age = %d WHERE city = '%s'",
                newAge, city
            ));
        } else {
            ...
            double newPrice = 50 + (random.nextDouble() * 1000);
            engine.executeSQL(String.format(
                "UPDATE products SET price = %.2f WHERE category = '%s'",
                newPrice, category
            ));
        }
    }
    ...
    private void insertUser() {
        ...
        String city = getRandomCity();
        engine.executeSQL(String.format("INSERT INTO users VALUES (%d, '%s', %d, '%s')",
            id, name, age, city));
    }
    ...
    private void insertProduct() {
        ...
        String category = getRandomCategory();
        engine.executeSQL(String.format("INSERT INTO products VALUES (%d, '%s', %.2f, '%s')",
            id, name, price, category));
    }
    ...
    private void insertOrder() {
        ...
        int quantity = random.nextInt(10) + 1;
        engine.executeSQL(String.format("INSERT INTO orders VALUES (%d, %d, %d, %d)",
            id, userId, productId, quantity));
    }
    ...
    private void updateUser() {
        int id = random.nextInt(10000) + 1;
        int newAge = random.nextInt(60) + 20;
        engine.executeSQL(String.format("UPDATE users SET age = %d WHERE id = %d",
            newAge, id));
    }
    ...
    private void updateProduct() {
        ...
        double newPrice = 50 + (random.nextDouble() * 1000);
        engine.executeSQL(String.format("UPDATE products SET price = %.2f WHERE id = %d",
            newPrice, id));
    }
    ...
    private void updateOrder() {
        ...
        engine.executeSQL(String.format("UPDATE orders SET quantity = %d WHERE id = %d",
            newQuantity, id));
    }
}
