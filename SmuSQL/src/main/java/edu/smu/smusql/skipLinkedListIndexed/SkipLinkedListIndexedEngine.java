...
public class SkipLinkedListIndexedEngine implements IEngine {
    ...
    public String create(String[] tokens) {
        ...
        String[] colVals = Arrays.stream(
                queryBetweenParentheses(tokens, 3).toLowerCase().split(",")).map(x -> x.trim())
            .toArray(String[]::new);
    ...
    public String insert(String[] tokens) {
        ...
        String valueList = queryBetweenParentheses(tokens,
            4); // Get values list between parentheses
        List<String> values = Arrays.asList(
            valueList.split(",")); // These are the values in the row to be inserted.
        ...
        if (tokens.length > 4 && tokens[4].equalsIgnoreCase("WHERE")) {
            ...
            for (int i = 5; i < tokens.length; i++) {
                if (tokens[i].equalsIgnoreCase("AND") || tokens[i].equalsIgnoreCase("OR")) {
                    whereClauseConditions.add(
                        new String[]{tokens[i].toUpperCase(), null, null, null});
                } else if (isOperator(tokens[i])) {
                    // Add condition with operator (column, operator, value)
                    String column = tokens[i
                        - 1].toLowerCase(); // Handle column name case-insensitively
            ...
            if (whereClauseConditions.size() == 1) {
                ...
                if (requirements[1].equalsIgnoreCase("ID")) {
                    ...
                    keysToSelect.addAll(
                        t.returnKeysByRequirementsOnIndex(requirements[1], requirements[2],
                            requirements[3]));
                }
    ...
    public String update(String[] tokens) {
        ...
        if (rowCount == 0) {
            return rowCount + " row(s) updated, not found";
        }    
    ...
    public String delete(String[] tokens) {
        ...
        if (rowCount == 0) {
            return rowCount + " row(s) deleted, not found";
        }
    ...
    private String formatSelectResults(List<Row> rows, List<String> columns) {
        ...
        for (Row row : rows) {
            ...
            for (String column : columns) {
                String value = rowData.get(column).trim();
                if (value.startsWith("'") && value.endsWith("'")) {
                    value = value.substring(1, value.length() - 1); // Remove surrounding single quotes
                }
